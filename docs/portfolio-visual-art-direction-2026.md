...
**Каждый визуал сначала объясняет решение и только потом работает на впечатление.**

Поэтому визуальная система разделена на два слоя, но их роли зависят от места на странице:

- растровые изображения на первых экранах кейсов отвечают за физический контекст, свет, материал и постановку объекта;
- HTML и CSS отвечают за подписи, статусы, данные, сценарии, схемы и обложки кейсов на главной.

На главной приоритет отдан не материальной метафоре, а короткой продуктовой истории. Посетитель ещё до открытия кейса видит проблему, ключевое правило решения и безопасный исход. Такой подход исключает сгенерированные псевдонадписи, сохраняет адаптивность и показывает проектное мышление, а не навык создания декоративных изображений.
...
Обложка кейса на главной больше не использует этот растровый файл. Она вручную собрана в HTML и CSS и показывает один критический момент: неизвестный штрихкод не добавляет товар, 86 уже принятых позиций остаются сохранены, завершение документа блокируется, а исключение получает владельца. За один взгляд считываются четыре свойства решения: сохранённый прогресс, честная ошибка, запрет опасного действия и управляемая передача ответственности.
...
Обложка KUBTEL на главной также реализована без растрового фона. В ней один и тот же инцидент показан одновременно в личном кабинете и у поддержки: абонент видит текущий этап и срок следующего сообщения, оператор уже имеет адрес и контекст, а восстановление завершается подтверждением результата. Это визуальный тезис кейса, а не абстрактная метафора связи.
...
Главные растровые изображения первых экранов кейсов созданы встроенным ImageGen. Сгенерированный текст в изображениях запрещён; все подписи добавлены в коде. Обложки обоих кейсов на главной и сквозные внутренние сценарии не создавались генеративной моделью: они вручную собраны в HTML и CSS, чтобы содержание, геометрия и адаптивное поведение оставались точными.
...
- Кейсы имеют одинаковую структуру: короткое визуальное доказательство сверху, контекст и переход к полной истории снизу.
- Обложка «Склада 15» показывает сохранённый прогресс, критическое исключение и запрет опасного завершения.
- Обложка KUBTEL показывает один статус в кабинете и поддержке, обещанный срок обновления и подтверждённое восстановление.
- У кейсов разные продуктовые сюжеты, но единая визуальная грамматика: фактический интерфейс, один главный конфликт, один безопасный исход и спокойная светлая сетка.
...
- Сгенерированный материальный маршрут используется только на первом экране самого кейса.
- На главной и внутри ключевого перехода используются настоящие HTML-карточки статуса, кабинета и поддержки: весь значимый текст остаётся точным и доступным.
