...
**Сильная материальная сцена создаёт эмоцию, а точный код закрепляет смысл.**

Поэтому визуальная система разделена на два слоя, но их роли зависят от места на странице:

- растровые изображения на первых экранах и в обложках кейсов отвечают за физический контекст, свет, материал, драматургию и первое впечатление;
- HTML и CSS отвечают за точные подписи, статусы, данные, адаптивное кадрирование и внутренние смысловые схемы.

Обложка на главной должна одновременно выдерживать два теста: остановить взгляд как сильная иллюстрация и без длинной подписи объяснить реальный конфликт кейса. Поэтому декоративные метафоры заменены предметными сценами, а сгенерированный текст запрещён. Две небольшие HTML-плашки уточняют факт и результат, но не объясняют изображение вместо него.
...
- материальная основа для настоящих статусов «Скан принят» и «Есть исключения» на странице кейса.
...
Новая обложка главной: `site/assets/portfolio-sklad-receiving-story-v2-20260810.png` и оптимизированная WebP-версия.

Вместо терминала среди абстрактных карточек показана цельная приёмочная станция: принятые коробки и контейнер продолжают движение, единственный проблемный груз находится на отдельном коралловом лотке, исключение видно на экране старшего смены, а выход защищён воротами с замком. Визуал показывает сохранённый прогресс, изоляцию ошибки, владельца и запрет опасного завершения как одну физически понятную сцену.
...
Новая обложка главной: `site/assets/portfolio-kubtel-shared-incident-v2-20260810.png` и оптимизированная WebP-версия.

Вместо абстрактной цепочки показаны три реальные части одной услуги: домашний роутер и телефон абонента, открытый оптоволоконный шкаф с активным ремонтом и рабочий экран поддержки. Телефон и монитор повторяют один коралловый знак инцидента, а физический кабель связывает домашнюю услугу с техническим узлом. Так идея общего контекста становится видна без диаграммы.
...
Главные растровые изображения первых экранов и обложки кейсов созданы встроенным ImageGen под ручной арт-дирекшн и последовательную проверку артефактов. Сгенерированный текст в изображениях запрещён; точные подписи добавлены в коде. Сквозные внутренние сценарии по-прежнему собраны в HTML и CSS, потому что там важнее последовательность действий, доступность и адаптивная перестройка.
...
- Кейсы имеют одинаковую структуру: сильная контекстная иллюстрация сверху, контекст и переход к полной истории снизу.
- Обложка «Склада 15» показывает сохранённый поток, единственное изолированное исключение, владельца и физически понятную блокировку выхода.
- Обложка KUBTEL показывает один инцидент в доме, сетевом узле и поддержке; совпадающий статус связывает каналы.
- У кейсов разные предметные сюжеты, но единая визуальная грамматика: светлая студийная сцена, правдоподобные материалы, один коралловый конфликт и две точные HTML-подписи.
...
- Сквозная кодовая иллюстрация связывает неизвестный штрихкод, сохранение прогресса, центр исключений и безопасное завершение.

### KUBTEL

- На первом экране самого кейса остаётся спокойный материальный маршрут, а на главной используется новая предметная сцена общего сервисного контекста.
- Внутри ключевого перехода используются настоящие HTML-карточки статуса, кабинета и поддержки: весь значимый текст остаётся точным и доступным.
- Контраст «до / после», общая модель состояния и сервисная схема используют тот же семантический цветовой язык, что и главная.
